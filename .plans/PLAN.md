...
- [async-worktree-status](async-worktree-status.md) — load worktree `git status`
  asynchronously so rows render instantly and `*`/`↑`/`↓` annotations fill in progressively.
...
| [async-worktree-status](async-worktree-status.md) | Render worktree rows immediately from `ListWorktrees`, fetch `git status` asynchronously, fill in annotations as each returns | |
